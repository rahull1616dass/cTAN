...
trait MMIHelper {
  ...
  protected def checkForWordType[T<: WordTypes.Word](in: Event)(implicit tag: ClassTag[T]): Condition.Result = {
    in.name match {
      case SpeechEvents.token.name | SpeechEvents.processedToken.name | SpeechEvents.recognized.name =>
        var isValid = false
  ...
  def checkConfidence(confidence: Float)(in: Event, register: SValSet, triggeredArc: ArcRep, current: StateRep, previous: StateRep): Condition.Result = {
    in.name match {
      case SpeechEvents.token.name | SpeechEvents.processedToken.name | SpeechEvents.recognized.name =>
        val conf = in.values.firstValueFor(types.Confidence)
        val isValid = conf >= confidence
        ConditionResult(isValid)
      case _ => ConditionResult(doTransition = false)
    }
  }
